...
患者・病床・看護師の評価関数を、モデル内で `allocate` と共有します。呼び出し側は候補から病床と看護師を選び、Action を実行します。選択した組み合わせを再検査し、ontology-owned な Allocation と3本のリンクを原子的に作ります。患者・病床・看護師への参照はリンクに置き、外部キー属性として重複させません。

ソースの患者は `waiting`、病床は受入可能、N1 のソースの枠は1のままです。保存した計画が追加の業務状態になります。候補評価ではソースの予約・準備状態に既存の計画を合わせ、計画済み病床を除外し、看護師の枠から計画数を差し引きます。再検索すると、P1 は計画済みなので候補がなく、P4 は資源が消費されたので候補がありません。割当前の候補検索では、P1・P4 ともに B101・N1 を取得できます。しかし P1 の確定後に同じ資源を P4 へ割り当てようとすると、以前の検索結果から選んでいても拒否されます。候補の取得は予約ではありません。ソースの再読み込み後も計画とリンクを保持します。
...
モデルから `bed_search`・`nurse_search`・`allocate` と探索ツールを生成します。呼び出し元は患者や病床を選び、適合条件を実装せずに使えます。患者や受入確認の絞り込みはエージェント自身のコード実行環境で行い、候補の適合判定はモデルに置きます。共通 API と失敗時の契約は [IMPLEMENTATION.ja.md](../../docs/IMPLEMENTATION.ja.md) にあります。
