...
test('hospital pivots back only to confirmed waiting patients; candidate functions explain exclusions without writes', (t) => {
  ...
  assert.deepEqual(ids(approvedPatients.objects), ['P1', 'P4'])
  assert.deepEqual(ids(rt.intersect(waiting, approvedPatients).objects), ['P1', 'P4'])
  ...
  const nurses = rt.call('nurseSearch', { patientId: 'P1', bedId: 'B101' }, { actor })
...
test('hospital records one atomic plan; source readiness is unchanged and the plan consumes effective resources after refresh', (t) => {
  ...
  assert.equal(rt.call('bedSearch', { patientId: 'P1' }, { actor }).set.objects.length, 0)
  const forOther = rt.call('bedSearch', { patientId: 'P4' }, { actor })
  ...
  const nurse = rt.call('nurseSearch', { patientId: 'P4', bedId: 'B101' }, { actor }).assessments[0]
  assert.equal(nurse.reasons.some((r) => r.code === 'NO_CAPACITY'), true)
  const refused = rt.execute('allocate', otherPlan, { actor })
  ...
  assert.equal(rt.call('bedSearch', { patientId: 'P4' }, { actor }).set.objects.length, 0)
...
test('hospital refuses an inconsistent selected bed/nurse pair even when each appeared in a candidate search', (t) => {
  ...
  rt.load(integrate(sources))
  assert.deepEqual(ids(rt.call('bedSearch', { patientId: 'P1' }, { actor }).set.objects), ['B101', 'B105'])
  assert.deepEqual(ids(rt.call('nurseSearch', { patientId: 'P1', bedId: 'B101' }, { actor }).set.objects), ['N1'])
  assert.deepEqual(rt.call('nurseSearch', { patientId: 'P1', bedId: 'B105' }, { actor }).set.objects, [])
  const result = rt.execute('allocate', { ...plan, bedId: 'B105' }, { actor })
