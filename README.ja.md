...
設備の異常、入院の受入依頼、取引のアラート。現場では、起きたことを受けて情報を突き合わせ、「誰に・何に、どの根拠で対応するか」を決め、状況が変わるたびに判断と実行を繰り返します。ここでは、この業務の流れを **データ駆動オペレーション** と呼びます。

次の3例では、 Operational Ontology を活用して対応対象と根拠を特定し、Action で条件を再確認して、タスク・仮割当・調査ケースを記録するところまでを示します。
