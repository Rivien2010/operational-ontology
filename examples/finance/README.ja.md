...
デモでは、集合演算ごとに両方の入力と結果の型・ID・件数を表示します。和集合で全受取先 X・Y・W・Z を集めます。A と B の受取先の積集合は X・W、さらに C の受取先との積集合を取ると X が残ります。全受取先からこの共通集合を引いた差集合は Y・W・Z です。これらは口座の同一性を比較する操作であり、取引金額の集計ではありません。

最初から X を指定して探したのではなく、A・B・C の送金先を照合した結果、3口座すべてに共通する相手として見つかりました。モデルの Function `recipientSummary` では、集計を使う別の経路も示します。
