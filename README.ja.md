...
Palantir Foundry の Ontology は、このパターンの実装のひとつです。このリポジトリも実装パターンの一つで、一息に読み切れる規模の最小リファレンス実装です。目的は定義を正確かつ実行可能な形にすることで、フレームワークではありません。フォークして、自由にアイデアを応用してください。
...
デモの題材は、[このリポジトリの元になった記事](https://note.com/gura105/n/nfe927c169c6a)と同じシナリオです。ある企業が競合を買収し、スキーマもステータスの表現方法も異なる 2 つのレガシー受注システムを抱え込みます。数十行の SQL と小さな TypeScript のマッピングでこれらを統合し、その上に `Customer` / `Order` / `Product`、さらにどのソースシステムにもテーブルが存在しない型 `Note` をオントロジーとしてモデリングします。デモで確認できるのは次の動作です。
...
4. **システムオブレコードへの書き戻し（write-back）。** モデルは、すべての状態について「どのシステムが正本を持つか」を宣言する。状態は次の 3 種類に分かれる。
   ...
   <img src="./assets/authority-map.svg" alt="Order オブジェクトの authority map。status と total は上流の受注システムが正本を持つ source-backed で、統制された write-back 経路を通る。assignee と Note は ontology-owned で、オントロジー自身のデータストアが唯一の正本になる。集計値や件数は derived で、計算されるだけで書き込まれない。所有者が宣言されていない状態は禁止。">
...
- 出荷済みの注文までキャンセルできてしまうなら、それはただの書き込み API です。現実の業務ルールや制約が反映できていません。
...
4 つの性質は実装機構を規定しませんが、実装ごとに異なり、かつユーザーから観測できてしまう選択がいくつかあります。これらは明示的に宣言しなければなりません。項目は 4 つです。
...
MCP のツール群はモデルから生成されます。`search_order`、`traverse_customer_orders`、`cancel_order`、`read_audit_log` など、クエリの種類ごと・アクションごとに 1 ツールです。ツール一覧はスキーマから導出され、個々のツール呼び出しはインスタンスに作用します。ここから 2 つの帰結が生まれます。
...
定義がただの値であることにはメリットがあります。列挙でき、diff が取れ、バージョン管理できます。前述の MCP ツール群も、この定義から機械的に導出されています。

業務操作としての書き込み API は `Runtime.execute()` ただ 1 つです。適用か拒否かにかかわらず、呼び出しごとにアクションのインスタンスが 1 つ生成され、監査ログに記録されます。処理は常に次の順序です。
...
編集（edits）もデータです。種類は `modify`、`create`、そして `link` / `unlink`。つまりアクションはオブジェクトのインスタンスやそのプロパティだけでなく、リンクのインスタンスも変更できます。リンク型はスキーマ側に属し、この層では変わりません。個々のリンクはインスタンスであり、アクションを通してのみ変わります。多重度も検査され、1 つの注文に顧客が 2 人つくような `link` はランタイムが拒否します。「注文を別の顧客に付け替える」は unlink と link の組で、他のすべてと同じ事前条件のもと、原子的に適用されます。生成も同じゲートを通ります。デモの `addOrderNote` は、ontology-owned なノートを生成して注文にリンクするところまでを 1 つの原子的なプランで行います。削除はこの版ではスコープ外です（[ステータス](#ステータス)参照）。モデル自体の変更（新しいオブジェクト型やリンク型の追加）はスキーマ進化の話です（FAQ 参照）。
...
この型(スキーマ)とインスタンスの区別は、形式オントロジーで TBox と ABox と呼ばれる一般的なものです。それ自体はパターンの成立条件ではありませんが、パターンの構造を整理するうえでは有用です。Foundry でも 3 つの対すべてに同じ命名形式が使われています。

- object type と object
- link type と link
- action type と action

いずれも前者が「schema definition」、後者がそのインスタンスです。スキーマ側（型の定義に authority 宣言 `owned` / `writeback` と visibility を加えたもの）は、ただの値として git で管理されます。実行時には `Runtime.declarations` や MCP のツール群という形で参照できますが、変更はできません。インスタンス側はストアに格納され、アクションを通してのみ変わります。この区別に沿って言い換えると、4 つの性質はいずれもインスタンス側の統制を定めています。authority 宣言は、「インスタンス側の各状態について、正本を誰が持つか」をスキーマ側で表明するものであり、その宣言が状態ごとの変更経路を決めます。write-back が関わるのは、正本が上流にあると宣言された状態だけです。この区分は edits の語彙にも現れています。`modify` / `create` / `link` / `unlink` はすべてインスタンスへの操作であり、スキーマを変える edit は存在しません。性質 2 の「スキーマ進化が変えるのは『何を言えるか』であって『何が真か』ではない」も、同じ区別の別の表現です（適用されたアクションをインスタンスとして検索する可能性については FAQ 参照）。
...
このリスクを、3 つの仕組みで抑えます（[機構の詳細](./IMPLEMENTATION.ja.md#失敗時のセマンティクスの詳細)）。第一に、不正なデータはシステムオブレコードに届きません。アダプタが走る前に、編集プラン全体がコミットと同じコードでドライランされるからです。第二に、監査ログはどちら向きの失敗でもプラン全体を記録し、照合の材料になります。第三に、「すべての試行は監査される」が意味するのは、このランタイムが完了まで観測できた試行のすべてです。自分のストアの内側では、ランタイムはトランザクショナルです。アクションの編集と監査エントリは 1 つの SQLite トランザクションでアトミックコミットされ、拒否された試行も記録されます。
...
**再インデックスと編集。** ストアには、ソースからインデックスされた「ベース(Base)」と、その上にアクションが発行した「編集(Edit)」という 2 種類の状態があります。ソースは動き続けるので、再インデックスと編集を突合したときの挙動を必ず決めなければなりません。本実装は Foundry と同じ形の最小化です。スナップショットが供給できるのは source-backed な状態だけ。ontology-owned なプロパティへの編集はオーバーレイに退避・保持され、新しいベースの上に再適用されます。ontology-owned なオブジェクトとリンクには `load()` は一切触れません。そして、ontology-owned な編集を取り残してしまう再インデックスは丸ごと拒否され、以前の状態がそのまま残ります。それは照合上の判断であり、ランタイムは照合上の判断を黙って下さないからです。削除と編集がコンフリクトした場合や部分スナップショットを含む完全な規則は、[実装ノート](./IMPLEMENTATION.ja.md#再インデックスと編集)にあります。
...
- **インデクシング基盤は作らない。** デモの規模なら素朴なクエリで十分。スケールは実装バリエーションであって、パターンの性質ではない。v0 では結果セットに上限を設けていない。
...
**ナレッジグラフだって書き込めるのでは？** 書き込めます。SPARQL UPDATE で `status = 'cancelled'` に書き換えられますし、`WHERE` 句や SHACL のシェイプで条件付きにもできます。スキーマとインスタンスの両側も備わっています。欠けているのは 3 つ目の対、つまり action type とそのインスタンスです。トリプルストアが 1 つの第一級ユニットとして提供しないのは、契約の残りの部分です。名前のついた業務操作、機械可読な拒否、試行の監査証跡、システムオブレコードへの書き戻し。違いは能力ではなく（トリプルストアの上にすべて構築することは可能です）、モデルから何が名前付き・統制付き・第一級のものとして出てくるか、です。
...
**エージェントがリトライしたら？** 本実装に冪等キーはありません。リトライされた `cancelOrder` は自分自身の事前条件（`ORDER_ALREADY_CANCELLED`）に拒否されます。これはルールの働きによる自然な冪等性であって、保証ではありません。また、write-back と交錯するリトライは、ソース側で副作用を二重に適用する可能性があります。アクションが自然に冪等でない場合の最小の出発点は、params に invocation id を入れることです（他のすべてと同じく監査に載ります）。ただし、それで得られるのは相関までで、実際の重複排除にはその id への一意性チェックが必要です。冪等性は実装定義ですが、エージェントは実際にリトライするので、宣言する価値があります。これには構造上の理由があります。アクションのインスタンスを区別する基準は、引数ではなく個々の発生です。同じパラメータで 2 回 submit しても、同じ試行の繰り返しではなく、別々の 2 つのインスタンスになります。監査ログに試行ごとに 1 エントリが記録されるのはこのためで、冪等キーは、本来は発生によってしか識別されないアクションに、値に基づく同一性を人工的に与える仕組みです。

**過去のアクションをオブジェクトのように検索できないか？** できます。Foundry に実例があります。Foundry の[アクションログ](https://www.palantir.com/docs/foundry/action-types/action-log)は、submit のたびに `[LOG]` オブジェクトを生成します。このオブジェクトは、そのアクションが編集したすべてのオブジェクトにリンクされます。適用されたアクションがグラフに加わり、検索・走査・集計の対象になるということです。本実装が監査ログをオブジェクトグラフの外に置いているのは、機能を控えたからではなく、契約上の理由です。ログには、オブジェクト化となじまない契約が 3 つあります。1 つ目は、何もコミットしない試行 — 拒否やクラッシュ — まで記録することです。2 つ目は、書き込みが決して失敗してはならないことです。エンコードできない値はプレースホルダとして記録されます。「検証して拒否する」スキーマ検証とは正反対の契約です。3 つ目は、ログはランタイム自身がアクションを経由せずに直接書くことです。エントリを通常のオブジェクトにするなら、性質 2 への例外を宣言する必要が生まれます。将来の版でアクションインスタンスを検索可能にする場合、取り得る方法は置き換えではなく射影です。監査ログを、ランタイムが所有しインデックスによって再生されるもう 1 つのソースとして扱い、そこからアクションインスタンスのオブジェクトを導出します。derived な状態は決して書き込まれないため、不変性は自動的に保たれます。そして監査ログは、その基盤として残り続けます。Foundry 自身のアーキテクチャも同じ構成です。`[LOG]` の射影はプラットフォームの監査ログと併存するのであって、それを置き換えるものではありません。

**オントロジー自体は、どう変化していく？** スキーマ進化（新しいオブジェクト型、プロパティの変更、リンク型の廃止）は現実の問題ですが、フェデレーションと同じくスコープ外です。Foundry にはバージョニングとプロポーザルの機構があり、学術分野では ontology evolution として研究されています。このリポジトリが提供するのは、安全に進化するための前提条件です。モデルはただの値なので、他のコードと同じように diff を取り、バージョン管理し、レビューできます。つまり、この層には変更管理の経路が 2 つあります。インスタンスの変更は、実行時にアクションと監査ログによって変更管理されます。スキーマの変更は、開発時に diff とレビューによって変更管理されます。スキーマはストアには格納されず、コードとしてのみ永続化されます。これは制約ではなく、バージョニングと承認の機構を git に全面的に委ねるという選択です。スキーマの変更管理を実行時にやるとすれば、データとして表現し直すのは構造であって、ルールではありません。ルールはコードであり続けます。ドメインモデリングは一回きりの工程ではなく、モデルはビジネスに合わせて調整され続けます。
