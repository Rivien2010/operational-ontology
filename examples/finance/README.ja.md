...
デモでは、集合演算ごとに両方の入力と結果の型・ID・件数を表示します。A と B の受取先の積集合は X・W、さらに C の受取先との積集合を取ると X が残ります。これらは口座の同一性を比較する操作であり、取引金額の集計ではありません。

最初から X を指定して探したのではなく、A・B・C の送金先を照合した結果、3口座すべてに共通する相手として見つかりました。X から `incoming` を辿ると7件の取引が得られます。デモでは、起点と期間で絞って保存しておいた3つの取引集合を和集合でまとめ、X の入金取引との積集合を取ります。残るのは T1a・T1b・T3・T4 の4件です。前日の T8、午前の T10、D からの T9 は除外されます。この根拠を送金元へ pivot すると3口座、4件の金額を合計すると510万円になります。同じ4件の根拠IDを Action に渡します。

モデルの Function `recipientSummary` では、受取先全体を指標で比較する経路も示します。
...
Action は選んだ全取引について、現在も指定した起点からの送金で、対象期間内にあり、選択した受取先へ到達することを検査し、ontology-owned なケースとリンクを作ります。ソースの再読み込み後もケースとリンクを保持しますが、ソースレコードの内容を過去のまま固定した根拠スナップショットにはしません。
